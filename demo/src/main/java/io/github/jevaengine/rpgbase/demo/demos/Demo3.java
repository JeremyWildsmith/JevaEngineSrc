 ...
 * @author Jeremy
 */
public class Demo3 implements IState
{
	private static final String DEMO_MAP = "world/demo3.jmp";
	...
	public Demo3(IWindowFactory windowFactory, IWorldFactory worldFactory, IAudioClipFactory audioClipFactory)
	{
	...
	public void enter(IStateContext context)
	{
		...
		try
		{
			ISceneBufferFactory sceneBufferFactory = new TopologicalIsometricSceneBufferFactory(TILE_WIDTH, TILE_HEIGHT, false);
			...
			m_steerDriver.add(new AvoidanceBehavior(0.8F));
			
			m_dialogueController = new DialogueController(context.getWindowManager(), m_windowFactory, m_player, sceneBufferFactory);
	...
	private static class DialogueController implements IDisposable
	{
		...
		public DialogueController(WindowManager windowManager, IWindowFactory windowFactory, RpgCharacter host, ISceneBufferFactory speakerViewSceneBufferFactory)
		{
		...
		private class DialogueObserver implements IActionObserver
		{
			...
			public void listening(IDialogueListenerSession session)
			{
				try
				{
					final CharacterDialogueQuery query = new CharacterDialogueQueryFactory(m_windowManager, m_windowFactory, "ui/windows/characterDialogue.jwl", m_speakerViewSceneBufferFactory).create(session);
